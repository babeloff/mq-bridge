...
async fn test_grpc_tls_roundtrip() {
    ...
    if let Some(out_route) = routes.get_mut("grpc_to_memory") {
        if let mq_bridge::models::EndpointType::Grpc(ref mut cfg) = out_route.output.endpoint_type {
        *cfg = tls_helpers::grpc_server_config_with_tls(&cert_dir, "127.0.0.1:50051");
        }
    }
    if let Some(in_route) = routes.get_mut("memory_to_grpc") {
        ...
        *cfg = tls_helpers::grpc_client_config_with_tls(&cert_dir, "http://127.0.0.1:50051");
        }
    }
    ...
    out_route.deploy("grpc_to_memory").await.expect("Failed to deploy gRPC server");
    tokio::time::sleep(Duration::from_secs(2)).await;

    in_route.deploy("memory_to_grpc").await.expect("Failed to deploy memory_to_grpc route");

    // simple sanity check: send one message and ensure it's received
    let in_channel = in_route.input.channel().unwrap();
    in_channel.fill_messages(vec![mq_bridge::test_utils::generate_test_messages(1).pop().unwrap()]).await.unwrap();
