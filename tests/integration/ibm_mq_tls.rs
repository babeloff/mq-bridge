...
mod ibm_mq_tls {
    ...
    use mq_bridge::traits::{MessagePublisher, MessageConsumer};
    ...
    async fn test_ibm_mq_tls_roundtrip() {
        ...
        run_test_with_docker("tests/integration/docker-compose/ibm_mq_tls.yml", || async {
            ...
            let publisher = Arc::new(IbmMqPublisher::new(&cfg).await.expect("Failed to create publisher"));
            let mut consumer = IbmMqConsumer::new(&cfg).await.expect("Failed to create consumer");
